...
function ob_EasyMenu(ob_em_menu_id, ob_em_oat_id, ob_em_ui, ob_em_hz_o, ob_em_vt_o, ob_em_mc, ob_em_ds, ob_em_ds_mc, ob_em_ds_ec, ob_em_se, ob_em_ss)
{	
	// object id
	...
	{	
		...
		if (obj.id.toLowerCase().indexOf("_7r14l") == -1)
		{
			...
			obj.className = this.classes[obj.getAttribute("emtype")]["c"];
			...
			{
				ob_em_safeAddOnEvent("onmouseover", "document.getElementById('" + obj.id + "')", "document.getElementById('" + obj.id + "').mouseovercollection", "document.getElementById('" + obj.id + "').mouseoveradded", "mouseHover(document.getElementById('" + obj.id + "'), ev, true)", this.id);
				ob_em_safeAddOnEvent("onmouseout", "document.getElementById('" + obj.id + "')", "document.getElementById('" + obj.id + "').mouseoutcollection", "document.getElementById('" + obj.id + "').mouseoutadded", "mouseHover(document.getElementById('" + obj.id + "'), ev, false)", this.id);
				ob_em_safeAddOnEvent("onclick", "document.getElementById('" + obj.id + "')", "document.getElementById('" + obj.id + "').mouseclickcollection", "document.getElementById('" + obj.id + "').mouseclickadded", "menuItemClick(document.getElementById('" + obj.id + "'), ev)", this.id);
			}
			else
			{
...
function ob_em_attachToControls ()
{
	...
	ob_em_safeAddOnEvent("oncontextmenu", "document", "document.ob_em_contextmenucollection", "document.ob_em_contextmenuadded", "documentMouseDown(ev)", this.id);
...
function ob_em_attachToControl (id)
{
	...
	if (ob_em_oat != null)
	{
		...
		if (pm != null && pm.menu != null && typeof(pm.menu.tagName) != 'undefined' && pm.menu.tagName.toLowerCase() == 'div' && pm.id != this.id)
		{
			...
			if (this.parentItem.firstChild.firstChild.firstChild.firstChild.emtype == 'm') this.parentItem.firstChild.firstChild.firstChild.firstChild.style.display='block';
...
function ob_em_detachFromControl (id)
{
	...
	if (ob_em_oat != null)
	{
		...
		if (pm != null && pm.menu != null && typeof(pm.menu.tagName) != 'undefined' && pm.menu.tagName.toLowerCase() == 'div' && pm.id != this.id)
		{
			...
			if (this.parentItem.firstChild.firstChild.firstChild.firstChild.emtype == 'm') this.parentItem.firstChild.firstChild.firstChild.firstChild.style.display='none';
...
function ob_em_documentMouseDown (e)
{
	...
	else if (this.mouseOverMenu(e,null)) return true;
 	// hide the menus
 	else 
 	{
 		this.rightClickHideMenu = true;
...
function ob_em_mouseHover (obj, e, over)
{
	...
	obj.className = this.classes[obj.getAttribute("emtype")]["c" + (over ? "_o" : "")];
	...
	try
	{
		...
		tmpObj.className = this.classes[obj.getAttribute("emtype")]["c_" + tmpObj.getAttribute("emtype") + (over ? "_o" : "")];
...
function ob_em_showMenu(e)
{
	...
	// if element is a treenode, select it on menu show
	...
	if (this.showEvent.toLowerCase() != "mouseover" || !this.mouseOverMenu(e)) ob_em_hideAllMenus(e, false, this.parentMenu != null ? this : null);
    ...
    if (!isOpera)
    {
		...
		this.cover_iframe.style.visibility = "visible";
	}
...
function ob_em_hideMenu(e, withVerify, force)
{
	...
	if (force || (menu.style.visibility == 'hidden' || (this.showEvent != 'mouseover' || (e && (!withVerify || !isMouseOverMenu) ))))
	{
		// if the iframe is not set, try to set it
		if (typeof this.cover_iframe == 'undefined' || this.cover_iframe == null) this.cover_iframe = document.getElementById('obout_easymenu_iframe_' + menu.id);
		// hide the iframe
		try {this.cover_iframe.style.display = "none";this.cover_iframe.style.visibility = "hidden";} catch (ex){};
...
function ob_em_menuItemClick (menuObject, menuEvent)
{
	...
	// hide the menu
	this.rightClickHideMenu = true;
	if (menuObject.getAttribute("nohide") == null || menuObject.getAttribute("nohide").toString().toLowerCase() != "nohide") this.hideMenu(menuEvent, false, false);
	...
	if (menuObject.firstChild.tagName.toLowerCase() == "a")
		if (document.all) menuObject.firstChild.click();
...
function ob_em_safeAddOnEvent(eventName, object, collection, eventAdded, functionToExecute, menuId)
{
	...
	else if (eval("obj." + eventName))
	{
		...
		var fct = function (ev) {eval(menuId + "." + functionToExecute);};
...
function ob_em_getMenuCoords (e, returnCurrentIfVisible)
{
	...
	if (this.underlay.style.visibility == "hidden" || returnCurrentIfVisible)
	{
		...
		// set coords out of the screen so that it won't flash while computing coords
		if (this.underlay.style.visibility == "hidden")
		{
			this.underlay.style.left = -2000;
			this.underlay.style.top = -2000;
		}
...
var isOpera=navigator.userAgent.indexOf("Opera")!=-1 && parseInt(navigator.userAgent.charAt(navigator.userAgent.indexOf("Opera")+6))>=8;