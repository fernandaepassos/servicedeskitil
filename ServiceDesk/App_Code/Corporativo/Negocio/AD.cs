...
namespace SServiceDesk.Negocio
{
    ...
    public class AD
    {
        ...
        private static string ADPath = string.Empty;
        private static string ADUser = "SDKITIL";
        private static string ADPassword = "srv%$#user123";
        ...
        /// <param name="Password"></param>
        /// <returns></returns>
        public static AD.LoginResult Login(string UserName, string Password)
        {
        ...
        /// <summary>
        /// Metodo que retorna todos os grupos de uma usuário
        /// </summary>
        /// <param name="UserName"></param>
        /// <param name="Password"></param>
        /// <returns></returns>
        public static DataSet GetUserGroups(string UserName)
        {
            DataSet dsGroup = new DataSet();

            //DirectoryEntry de = GetUser(UserName);

            //groups = de.Invoke("Groups",null);

            //Create a new table object within the dataset
            DataTable tbGroup = dsGroup.Tables.Add("UserGroups");
            tbGroup.Columns.Add("GroupName");
            tbGroup.Columns.Add("Guid");

            DataRow rwGroup = tbGroup.NewRow();
            rwGroup["GroupName"] = "Administradores";
            rwGroup["Guid"] = "1";
            //append the row to the table of the dataset
            tbGroup.Rows.Add(rwGroup);

            rwGroup = tbGroup.NewRow();
            rwGroup["GroupName"] = "Usuários";
            rwGroup["Guid"] = "2";
            //append the row to the table of the dataset
            tbGroup.Rows.Add(rwGroup);
            ...
            //
            //				distName = (string)groupEntry.Properties["distinguishedName"].Value;
            //				startPos = distName.IndexOf("CN=")+3;
            //				endPos = distName.IndexOf(",",startPos);
            //
            //				rwGroup["GroupName"] = distName.Substring(startPos,endPos-startPos);
            //				rwGroup["Guid"]= groupEntry.NativeGuid;
            //
            //				//append the row to the table of the dataset
            //				tbGroup.Rows.Add(rwGroup);
            //			}
            dsGroup.Tables[0].DefaultView.Sort = "GroupName";
            return dsGroup;
        ...
        public static DataSet GetGroups()
        {
            ...
            if (results.Count > 0)
            {
                ...
                foreach (SearchResult Result in results)
                {
                    ...
                    rwGroup["Guid"] = Result.GetDirectoryEntry().Guid;
        ...
        public static bool SetUserPassword(string UserName, string OldPassword, string NewPassword)
        {
            ...
            try
            {
                ...
                if (AD.Login(UserName, OldPassword) == AD.LoginResult.LOGIN_OK)
                {
        ...
        public static void EnableUserAccount(DirectoryEntry oDE)
        {
            ...
            oDE.Properties["userAccountControl"][0] = AD.ADAccountOptions.UF_NORMAL_ACCOUNT | AD.ADAccountOptions.UF_DONT_EXPIRE_PASSWD;
            oDE.CommitChanges();
        ...
        public static void DisableUserAccount(DirectoryEntry oDE)
        {
            ...
            oDE.Properties["userAccountControl"][0] = AD.ADAccountOptions.UF_NORMAL_ACCOUNT | AD.ADAccountOptions.UF_DONT_EXPIRE_PASSWD | AD.ADAccountOptions.UF_ACCOUNTDISABLE;
            oDE.CommitChanges();
        ...
        private static string GetLDAPDomain()
        {
            ...
            string[] LDAPDC = ClsParametro.ADServer.Split('.');
